...
从 GitHub Releases 下载对应的可执行文件和 `.sha256` 文件：
...
运行前校验 SHA-256。macOS 和 Linux 需要先添加执行权限：
...
`pnpm verify` 执行格式、类型、Lint、单元、集成和浏览器检查。`pnpm build` 在 `build/artifacts` 中生成当前平台的可执行文件和 SHA-256。
