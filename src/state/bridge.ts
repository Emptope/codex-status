...
export async function command<T>(name: string, args: Record<string, unknown> = {}): Promise<T> {
  ...
  const readOnly = name === 'snapshot' || name === 'preferences';
  const response = await fetch(`/api/${name}`, {
    method: readOnly ? 'GET' : 'POST',
    headers: { 'Content-Type': 'application/json' },
    ...(readOnly ? {} : { body: JSON.stringify(args) }),
  });
...
async function resize(width: number | undefined, height: number) {
  if (native) await command('resize', { width: width ?? innerWidth, height });
}
export async function resizeHeight(height: number) {
  await resize(undefined, height);
}
