...
| 交付 | 各平台原生构建、GitHub Actions | Windows、macOS、Linux 分开验证和打包 |
...
| 平台 | 首版目标 | 验证重点 |
| --- | --- | --- |
| Windows 11 x64 | 原生窗口、托盘、置顶、安装包 | WebView2、多 DPI、任务栏、睡眠恢复 |
| Windows + WSL2 | Windows 原生悬浮窗观察选定发行版的 CLI | 在发行版内定位数据根；通过 `wsl.exe` 参数数组连接，不依赖高频扫描网络路径 |
| macOS 14+ arm64 / x64 | 原生窗口、菜单栏入口、签名应用包 | 多桌面、全屏窗口、权限和焦点行为 |
| Ubuntu 22.04 / 24.04 x64，X11 | 原生窗口、可用环境中的托盘与置顶 | WebKitGTK、桌面依赖、字体、安装包 |
...
| 指标 | 初始目标 | 计量约定 |
| --- | --- | --- |
| 应用分发包 | 不超过 20 MiB | 单架构压缩包；另列首次安装所需系统组件与已有 Codex 大小，不隐去额外下载 |
...
- Cargo target、Vite 输出和打包中间产物统一置于 `build` 下；依赖下载缓存可以复用，编译产物不跨构建复用。
- 一次完整构建流水线内先生成前端，再编译和打包桌面端，依赖阶段使用本次生成的文件；重新开始构建则重新清理。
- 开发监听在一个已清理的任务内增量工作，重新启动开发任务前清理。打包与验证不得借用正在运行的开发产物。
...
交付：各平台产物、安装与卸载说明、兼容说明、许可证清单、版本记录。

验收：

- 各平台执行清理后的完整验证与发行构建，记录 Codex 支持版本和实际运行依赖。
- Windows 安装包、macOS 应用包、Linux 包在干净环境安装、启动和卸载通过；应用升级不覆盖 Codex 数据。
